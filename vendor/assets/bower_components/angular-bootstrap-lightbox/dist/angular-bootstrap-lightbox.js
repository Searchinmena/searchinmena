...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><button class=close aria-hidden=true ng-click=$dismiss()>×</button><div class=btn-group><a class=\"btn btn-xs btn-default\" ng-click=Lightbox.prevImage() translate>‹ Previous</a> <a class=\"btn btn-xs btn-default btn-next\" ng-click=Lightbox.nextImage() translate>Next ›</a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><div ng-if=Lightbox.showVideo class=\"embed-responsive embed-responsive-16by9\"><embed-video lightbox-video lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} controls showinfo=0 title=0 byline=0 portrait=0 info=0 badge=0 class=embed-responsive-item></embed-video></div><img ng-if=!Lightbox.showVideo lightbox-src={{Lightbox.imageUrl}} alt=\"\"></div></div>"
  );
...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.$get = ['$document', '$injector', '$modal', '$timeout', 'ImageLoader',
     ...
     * @memberOf bootstrapLightbox.Lightbox
     */
    Lightbox.openModal = function (newImages, newIndex, options) {
      Lightbox.images = newImages;
      Lightbox.setImage(newIndex);

      // store the modal instance so we can close it manually if we need to
      Lightbox.modalInstance = $modal.open(angular.extend(options, {
