class BusinessStoringHandler < BaseService
  ...
  :business_types_storing_handler

  def initialize(business, business_params, tags_params, business_types_params,
    validator)
    ...
    self.business_types_storing_handler = BusinessTypesStoringHandler.new(
      business, business_types_params)
  end

  def perform
    success = [storing_handler,
               tags_storing_handler,
               business_types_storing_handler].map(&:perform).all?
